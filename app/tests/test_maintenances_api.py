# app/tests/test_maintenances_api.py
from fastapi.testclient import TestClient


def get_token_for(
    client: TestClient, email: str = "m@test.de", password: str = "supersecret"
) -> str:
...
def create_bike(client: TestClient, token: str, brand: str = "Canyon") -> int:
...
def test_crud_owner_scoped(client: TestClient) -> None:
    ...
    # update
    r = client.put(
        f"/api/v1/maintenances/{mid}", json={"is_external": False, "vendor": None}, headers=h1
    )
    ...
    assert (
        client.put(f"/api/v1/maintenances/{mid}", json={"vendor": "Y"}, headers=h2).status_code
        == 404
    )

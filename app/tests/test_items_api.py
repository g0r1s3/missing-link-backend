...
def _get_token(
    client: TestClient, email: str = "test@example.com", password: str = "secret123"
) -> str:
...
def test_create_and_get_via_api(client: TestClient) -> None:
...
def test_list_filter_pagination_api(client: TestClient) -> None:
    token = _get_token(client)
    client.post(
        "/api/v1/items", headers={"Authorization": f"Bearer {token}"}, json={"name": "Pumpe A"}
    )
    client.post(
        "/api/v1/items", headers={"Authorization": f"Bearer {token}"}, json={"name": "Schlauch"}
    )
    client.post(
        "/api/v1/items", headers={"Authorization": f"Bearer {token}"}, json={"name": "Pumpe B"}
    )
...
def test_update_and_delete_api(client: TestClient) -> None:
    token = _get_token(client)
    item_id = client.post(
        "/api/v1/items",
        headers={"Authorization": f"Bearer {token}"},
...
def test_not_found_paths(client: TestClient) -> None:
    ...
    assert (
        client.put(
            "/api/v1/items/999999", headers={"Authorization": f"Bearer {token}"}, json={"name": "X"}
        ).status_code
        == 404
    )
    assert (
        client.delete(
            "/api/v1/items/999999", headers={"Authorization": f"Bearer {token}"}
        ).status_code
        == 404
    )

...
def get_token_for(client, email="a@test.de", password="supersecret"):
...
def test_bike_crud_requires_auth(client):
    r = client.post("/api/v1/bikes", json={"brand": "Canyon"})
    assert r.status_code == 401

def test_bike_crud_happy_path(client):
    token = get_token_for(client)
    hdr = {"Authorization": f"Bearer {token}"}

    r = client.post("/api/v1/bikes", json={"brand": "Canyon", "model": "Endurace", "year": 2022}, headers=hdr)
    assert r.status_code == 201
    bike = r.json(); bike_id = bike["id"]
...
def test_bike_is_owner_scoped(client):
    ...
    assert client.put(f"/api/v1/bikes/{bike_id}", json={"brand": "X"}, headers=hdr_b).status_code == 404

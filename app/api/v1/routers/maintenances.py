...
@router.get("", response_model=List[MaintenanceRead], summary="List maintenances (optionally for a specific bike)")
async def list_maintenances(
    bike_id: Optional[int] = Query(default=None, description="Filter by bike_id"),
...
async def update_maintenance(
    ...
    m = await service.update(session, owner_id=current.id, maintenance_id=maintenance_id, payload=payload)
    if not m:
        raise HTTPException(status_code=404, detail="Maintenance not found")
    return MaintenanceRead.model_validate(m.__dict__)

...
async def _ensure_bike_owned(session: AsyncSession, bike_id: int, owner_id: int) -> bool:
    q = await session.execute(select(Bike.id).where(and_(Bike.id == bike_id, Bike.owner_id == owner_id)))
    return q.scalar_one_or_none() is not None

async def list_for_user(session: AsyncSession, owner_id: int, bike_id: int | None = None) -> Sequence[Maintenance]:
    stmt = select(Maintenance).join(Bike, Maintenance.bike_id == Bike.id).where(Bike.owner_id == owner_id)
...
async def get_for_user(session: AsyncSession, owner_id: int, maintenance_id: int) -> Maintenance | None:
...
async def create(session: AsyncSession, owner_id: int, payload: MaintenanceCreate) -> Maintenance | None:
...
async def update(session: AsyncSession, owner_id: int, maintenance_id: int, payload: MaintenanceUpdate) -> Maintenance | None:
